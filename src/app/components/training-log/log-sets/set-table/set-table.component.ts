...
export class SetTableComponent implements OnInit {
  ...
  index = 0;
  pageSize = 5;
  displayedColumns = ["reps", "number", "measureUnit", "date"];
  exercise: Exercise;

  constructor(private setService: SetService) {}

  ngOnInit(): void {
    this.exerciseSubject.subscribe(exercise =>  {this.exercise = exercise, this.getExercisesPageFromUnit(this.pageSize, 0, exercise)});
   
  }

  public turn(event: PageEvent) {
    this.getExercisesPageFromUnit(event.pageSize, event.pageIndex, this.exercise);
    this.index = event.pageIndex;
  }

  private getExercisesPageFromUnit(size: number, page: number, exercise:Exercise): void {
    this.setsPage$ = 
