...
export class UserinfoFormComponent implements OnInit {
  ...
  deleteAccount(){
    if(confirm("Beim Fortfahren werden ALLE deine gespeicherten Daten permanent gelöscht. Möchten Sie diesen Account löschen")){
      this.userinfoService.deleteAccount().subscribe(() => {
        this.router.navigate(["/login"])
        this.authenticationService.setAuthentication(false);
        localStorage.removeItem("jwt")
        this.alertService.openSnackBar("Deine Daten wurden erfolgreich gelöscht!", "Ok")
      })
    }
  }
